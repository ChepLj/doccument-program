import { MenuItem, ListSubheader, FormControl, InputLabel, Select, TextField, Box, Typography, List, ListItem, Button, Snackbar, Alert } from "@mui/material";
    ...
    const createIdCodeJSXList = () => {
      ...
      prop?.idCodeFieldList.forEach((motherCrr: any, motherIndex: number) => {
        ...
        motherCrr?.Current.forEach((element: any, index: number) => {
          idCodeListJSX.push(
            <MenuItem key={`idCodeItem-${Math.random()}`} value={JSON.stringify(element)} color="secondary">
              <span style={{ color: "red" }}>{element.idCode}</span>
            </MenuItem>
          );
        });
      });
        ...
        <FormControl required sx={{ m: 1, Width: 20 }} size="small">
          ...
          >
            ...
            <MenuItem value={"PROGRAM"}>
              <span style={{ color: "violet" }}>PROGRAM</span>
            </MenuItem>
          </Select>
